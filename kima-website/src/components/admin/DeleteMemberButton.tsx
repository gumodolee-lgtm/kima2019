...
export function DeleteMemberButton({ userId, userName }: Props) {
  ...
  if (confirming) {
    return (
      <div className="flex items-center gap-1.5">
        <span className="text-xs text-red-600 font-medium">{userName} 삭제?</span>
        <button
          type="button"
  ...
  return (
    <button
      ...
      className="px-2 py-0.5 rounded text-xs border border-red-200 text-red-500 hover:bg-red-50 hover:border-red-400 transition-colors"
    >
      삭제
    </button>
  )
